...
style = ttk.Style()
style.theme_use('default')# ('winnative', 'clam', 'alt', 'default', 'classic', 'vista', 'xpnative')
...
def analyze_clicked():
    ...
    if len(cleaned_text) !=0:
        ...
        for item in tree3.get_children():
            tree3.delete(item)
        for sentence,m,dec,knnC,LOGREs,randomF,svc_lin in zip(cleaned_text,classes_mnb,decision_t,knn_c,logRe,random_f,linear):
            if sentence != '':
                tree2.insert('', 'end', text="1", values=(sentence,po_ne(LOGREs),po_ne(m),po_ne(knnC),po_ne(dec),po_ne(randomF),po_ne(svc_lin)))
                ...
                tree3.insert('', 'end', text="1", values=(sentence,'pos' if (m+dec+knnC+LOGREs+randomF+svc_lin)>=4 else 'neg'))
...
tree.grid(row=0,column=0,pady=10,padx=0)

tree2 = ttk.Treeview(third_notebook,column=("Sentence/Algorithm",'Logestic',"Multinomial NB","KNN","Decision Tree","Random Forest",'SVC'), show='headings', height=9)
tree2.column("# 1")
...
tree2.column("#6", anchor=CENTER,width=120)
tree2.heading("#6",  text="Random Forest")
tree2.column("# 7", anchor=CENTER,width=78)
tree2.heading("#7", text="SVC")
