...
def analyze_clicked():
    ...
    if len(cleaned_text) !=0:
        ...
        for item in tree3.get_children():
            tree3.delete(item)
        for sentence,m,b,dec,knnC,LOGREs,randomF,svc_lin in zip(cleaned_text,classes_mnb,classes_bnb,decision_t,knn_c,logRe,random_f,linear):
            if sentence != '':
                ...
                tree3.insert('', 'end', text="1", values=(sentence,'pos' if (m+b+dec+knnC+LOGREs+randomF+svc_lin)>=4 else 'neg'))
